# Fault: operador errado (subtracao em vez de soma ao acumular a faixa 3).
# A parcela da faixa 15% e' subtraida do total em vez de somada.
...
def income_tax(income: float) -> float:
    ...
    if inc > b2:
        part = min(inc, b3) - b2
        tax -= part * Decimal("0.15")  # DEFEITO: deveria ser tax +=
    if inc > b3:
        part = inc - b3
